      ...
      <div className="px-6 mx-auto lg:px-8">
        <div className={`-mx-6 grid gap-0.5 overflow-hidden sm:mx-0 sm:rounded-2xl ${columnClasses[cols]}`}>
          {options.map((option, index) => (
            <div 
              key={index} 
              className={`p-6 cursor-pointer bg-white/10 sm:p-4 hover:bg-white/20 transition-all duration-500 
            ...
            >
              ...
              {showLabel && (
                <span className="inline-flex items-center px-2 py-1 mt-2 text-lg font-light text-gray-100 rounded-md bg-gray-800/5 ring-1 ring-inset ring-gray-500/10">
                    {option.name}
                </span>
              )}
            </div>
